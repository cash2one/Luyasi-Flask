...
class FlaskTestCaseMixin(object):
    ...
    #目前这个好像还不好用,算法对不上，所以在测试的时候直接把csrf功能关了.
...
#         csrf_token = csrf_build
    ...
    def _request(self, method, *args, **kwargs):
        ...
        if hasattr(self, 'cookies') and self.cookies != None:
            self.client.set_cookie(self.app.config['KINORSI_SERVER_NAME'], 'session', self.cookies['session'])
