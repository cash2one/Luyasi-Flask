...
class JobForm(Form):
	...
	content = CKEditorField(gettext(u'Job detail'), import_js=True, validators=[CKEditorRequired(message=gettext('The comment forgot you~'))],
...
class JobReportForm(Form):
	...
	id = IntegerField(u'id', default=0, widget=HiddenInput())
	content = TextAreaField(gettext(u'Report content'), validators=[Required(), Length(min=30)])
	captcha = CaptchaField()
