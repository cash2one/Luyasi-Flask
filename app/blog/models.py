...
class Comment(db.Model, ModelVersion, JsonSerializer):
    ...
    # 用来记住第一楼用的
    first_comment_id = db.Column(GUID(), db.ForeignKey('blog_comment.id'))
    first_comment = db.relationship('Comment', foreign_keys=[first_comment_id], remote_side=[id])

    blog_id = db.Column(GUID(), db.ForeignKey('blog_blog.id'))
    ...
    # ref comment
    ref_comment_id = db.Column(GUID(), db.ForeignKey('blog_comment.id'))
...
class Category(db.Model, ModelVersion, JsonSerializer):
    ...
    parent_id = db.Column(GUID(), db.ForeignKey('blog_category.id'))
